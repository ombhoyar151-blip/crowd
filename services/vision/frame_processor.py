import logging
import time
from pathlib import Path
from typing import Callable, Optional

import numpy as np

from config.settings import Settings
from models.detection import Detection, DetectionBatch
from services.vision.detector import Detector
from services.vision.ingestion import StreamIngestion

logger = logging.getLogger(__name__)

FrameCallback = Callable[[np.ndarray, DetectionBatch], None]


class FrameProcessor:
    def __init__(self, settings: Settings):
        self.settings = settings
        self.ingestion: Optional[StreamIngestion] = None
        self.detector: Optional[Detector] = None
        self._callback: Optional[FrameCallback] = None
        self._running = False

    def initialize(self):
        output_dir = Path(self.settings.output_dir)
        output_dir.mkdir(parents=True, exist_ok=True)

        source_type = self.settings.video_source
        self.ingestion = StreamIngestion(
            source_type=source_type,
            max_queue_size=self.settings.max_queue_size,
            target_fps=self.settings.target_fps,
            source_path=self.settings.video_path,
            rtsp_url=self.settings.rtsp_url,
            webcam_id=self.settings.webcam_id,
        )

        self.detector = Detector(
            model_path=self.settings.model_path,
            conf_threshold=self.settings.conf_threshold,
            iou_threshold=self.settings.iou_threshold,
            device=self.settings.device,
            class_filter=self.settings.class_filter,
        )

        logger.info("FrameProcessor initialized")

    def set_callback(self, callback: FrameCallback):
        self._callback = callback

    def run(self, stop_event=None):
        if not self.ingestion or not self.detector:
            raise RuntimeError("FrameProcessor not initialized")

        self.ingestion.start()
        self._running = True
        camera_id = Path(self.settings.video_path).stem if self.settings.video_source == "file" else "stream"

        logger.info("FrameProcessor running (camera=%s)", camera_id)

        while self._running:
            if stop_event and stop_event.is_set():
                break

            frame = self.ingestion.read()
            if frame is None:
                time.sleep(0.001)
                continue

            frame_number = self.ingestion.frame_count
            timestamp = time.time()

            detections = self.detector.detect(
                frame,
                camera_id=camera_id,
                frame_number=frame_number,
            )

            batch = DetectionBatch(
                camera_id=camera_id,
                frame_number=frame_number,
                timestamp=timestamp,
                detections=detections,
            )

            if self._callback:
                self._callback(frame, batch)

        self.ingestion.stop()
        logger.info("FrameProcessor stopped")

    def stop(self):
        self._running = False
